...
const FeatureRightSideInner = forwardRef(({ data }, ref) => {
  ...
  return (
    <div ref={ref} className="w-full px-8 bg-[#FAFAFA]">
      <div
        className={`flex flex-col md:flex-row  md:pt-4 md:pb-0 top-20`}
      ...
      >
        ...
        <div className="md:w-1/2 text-black md:pr-10 pr-4 md:sticky  bg-[#FAFAFA] z-10">
          ...
          {isFirst && (
            <div className="flex space-x-4 mt-4">
              <motion.button
                onClick={() =>
                  handleModeChange(mode === "dark" ? "light" : "dark")
                }
                className={`
        ...
        <div
          className={`md:w-1/2 md:relative md:pt-4 ${
            isFourth ? "min-h-[300px]" : ""
          }`}
        >
          {isThird ? (
            <video
              autoPlay
              muted
              className={`object-cover rounded-[30px] w-[431px]  ${
                imageSize ?? "h-[518px]"
              }`}
            >
            ...
            <img
              src={imageSrc}
              alt={title}
              className={`object-cover w-[492px]  absolute -bottom-[10%] ${
                imageSize ?? "h-[378px]"
              }`}
            />
          ) : isSecond ? (
