...
const ReadersNLTest = ({ count, email, setEmail, mHandleSubmit }) => {
  ...
  useEffect(() => {
    if (count > 0) {
      ...
      setTimeout(() => setShowSection(false), 300);
    }
  }, [count]);
  ...
  const handleSubmit = () => {
    if (!email) {
      ...
      setError("");
      mHandleSubmit();
      setSubmitted(true); // Show thank-you message here
    }
  };
          ...
          <>
            ...
            <div className="flex flex-col w-full space-y-4">
              <input
                ...
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                className="border border-white px-4 py-2 bg-black text-white rounded-md text-sm"
                placeholder="Email address"
              />
              <button
                onClick={handleSubmit}
