    ...
    <div className="block lg:hidden relative w-full h-full flex justify-center items-center ">
              ...
              <div className="w-full space-y-2 mx-2">
                {chartDatas.map((data) => (
                  <div key={data.id} className="flex items-center ">
                    ...
                    <div className="text-[8px] text-[#515151] w-8 ml-4">{data.percentage}</div>
                    
                  
                    <div className={`text-[8px] ml-4  ${data.livePercentage.includes('+') ? 'text-green-500' : 'text-red-500'}`}>
                      {data.livePercentage}
                    </div>
                  </div>
                ))}
      ...
      <Card
            ...
            >
              ...
              <p className="text-[#FAFAFA] font-Manrope font-[400px] leading-[1.5]  max-w-[550px] text-[10px] pb-4 pt-4">Etiam et convallis dolor. Praesent pulvinar molestie leo, in finibus felis porttitor eget. Suspendisse vel aliquet tellus. Cras ac venenatis mi. Maecenas lobortis nisi enim, eget accumsan risus viverra in. Nunc semper neque porta ipsum porta dignissim.
              </p>
            </Card>
      </div>
    </div>
